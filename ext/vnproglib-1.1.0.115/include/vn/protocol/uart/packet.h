...
struct vn_proglib_DLLEXPORT Packet
{
	...
	/// \return The total number bytes in the generated command.
	static size_t genWriteFilterMeasurementsVarianceParameters(ErrorDetectionMode errorDetectionMode, char* buffer, size_t size, float angularWalkVariance, math::vec3f angularRateVariance, math::vec3f magneticVariance, math::vec3f accelerationVariance);
	...
	/// \return The total number bytes in the generated command.
	static size_t genWriteFilterBasicControl(ErrorDetectionMode errorDetectionMode, char* buffer, size_t size, uint8_t magMode, uint8_t extMagMode, uint8_t extAccMode, uint8_t extGyroMode, math::vec3f gyroLimit);
	...
	/// \return The total number bytes in the generated command.
	static size_t genWriteVpeMagnetometerAdvancedTuning(ErrorDetectionMode errorDetectionMode, char* buffer, size_t size, math::vec3f minFiltering, math::vec3f maxFiltering, float maxAdaptRate, float disturbanceWindow, float maxTuning);
	...
	/// \param[in] maxTuning The register's MaxTuning field.
	/// \return The total number bytes in the generated command.
	static size_t genWriteVpeAccelerometerAdvancedTuning(ErrorDetectionMode errorDetectionMode, char* buffer, size_t size, math::vec3f minFiltering, math::vec3f maxFiltering, float maxAdaptRate, float disturbanceWindow, float maxTuning);

	/// \brief Generates a command to read the VPE Gryo Basic Tuning register on a VectorNav sensor.
	///
	/// \param[in] errorDetectionMode The type of error-detection to use in generating the command.
	/// \param[in] buffer Caller provided buffer to place the generated command.
	/// \param[in] size Number of bytes available in the provided buffer.
	/// \return The total number bytes in the generated command.
	static size_t genReadVpeGryoBasicTuning(ErrorDetectionMode errorDetectionMode, char* buffer, size_t size);

	/// \brief Generates a command to write to the VPE Gryo Basic Tuning register on a VectorNav sensor.
	///
	/// \param[in] errorDetectionMode The type of error-detection to use in generating the command.
	/// \param[in] buffer Caller provided buffer to place the generated command.
	/// \param[in] size Number of bytes available in the provided buffer.
	...
	/// \param[in] adaptiveTuning The register's AdaptiveTuning field.
	/// \return The total number bytes in the generated command.
	static size_t genWriteVpeGryoBasicTuning(ErrorDetectionMode errorDetectionMode, char* buffer, size_t size, math::vec3f angularWalkVariance, math::vec3f baseTuning, math::vec3f adaptiveTuning);
	...
	/// \return The total number bytes in the generated command.
	static size_t genWriteFilterStartupGyroBias(ErrorDetectionMode errorDetectionMode, char* buffer, size_t size, math::vec3f bias);
	...
	/// \param[in] position The register's Position field.
	/// \return The total number bytes in the generated command.
	static size_t genWriteReferenceVectorConfiguration(ErrorDetectionMode errorDetectionMode, char* buffer, size_t size, uint8_t useMagModel, uint8_t useGravityModel, uint32_t recalcThreshold, float year, math::vec3d position);
	...
	/// \param[out] posUncertainty The position uncertainty value in the packet.
	/// \param[out] velUncertainty The velocity uncertainty value in the packet.
	void parseVNINE(double *time, uint16_t *week, uint16_t *status, math::vec3f *ypr, math::vec3d *position, math::vec3f *velocity, float *attUncertainty, float *posUncertainty, float *velUncertainty);
	...
	/// \param[out] acceleration The acceleration values in the packet.
	/// \param[out] angularRate The angular rate values in the packet.
	void parseVNISL(math::vec3f* ypr, math::vec3d* lla, math::vec3f* velocity, math::vec3f* acceleration, math::vec3f* angularRate);
	...
	/// \param[out] acceleration The acceleration values in the packet.
	/// \param[out] angularRate The angular rate values in the packet.
	void parseVNISE(math::vec3f* ypr, math::vec3d* position, math::vec3f* velocity, math::vec3f* acceleration, math::vec3f* angularRate);
	...
	void parseFilterMeasurementsVarianceParameters(float* angularWalkVariance, math::vec3f* angularRateVariance, math::vec3f* magneticVariance, math::vec3f* accelerationVariance);
	...
	void parseFilterBasicControl(uint8_t* magMode, uint8_t* extMagMode, uint8_t* extAccMode, uint8_t* extGyroMode, math::vec3f* gyroLimit);
	...
	void parseVpeMagnetometerAdvancedTuning(math::vec3f* minFiltering, math::vec3f* maxFiltering, float* maxAdaptRate, float* disturbanceWindow, float* maxTuning);
	...
	/// \param[out] disturbanceWindow The register's DisturbanceWindow field.
	/// \param[out] maxTuning The register's MaxTuning field.
	void parseVpeAccelerometerAdvancedTuning(math::vec3f* minFiltering, math::vec3f* maxFiltering, float* maxAdaptRate, float* disturbanceWindow, float* maxTuning);

	/// \brief Parses a response from reading the VPE Gryo Basic Tuning register.
	///
	/// \param[out] angularWalkVariance The register's AngularWalkVariance field.
	/// \param[out] baseTuning The register's BaseTuning field.
	/// \param[out] adaptiveTuning The register's AdaptiveTuning field.
	void parseVpeGryoBasicTuning(math::vec3f* angularWalkVariance, math::vec3f* baseTuning, math::vec3f* adaptiveTuning);

	/// \brief Parses a response from reading the Filter Startup Gyro Bias register.
	///
	/// \param[out] bias The register's Bias field.
	void parseFilterStartupGyroBias(math::vec3f* bias);
