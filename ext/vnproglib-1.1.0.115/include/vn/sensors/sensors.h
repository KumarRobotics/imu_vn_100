...
public:
	...
	void writeFilterMeasurementsVarianceParameters(
		...
		const math::vec3f &angularRateVariance,
		const math::vec3f &magneticVariance,
		const math::vec3f &accelerationVariance,
		bool waitForReply = true);
	...
	void writeVpeMagnetometerAdvancedTuning(
		const math::vec3f &minFiltering,
		const math::vec3f &maxFiltering,
	...
	/// \brief Reads the VPE Gryo Basic Tuning register.
	///
	/// \return The register's values.
	VpeGryoBasicTuningRegister readVpeGryoBasicTuning();

	/// \brief Writes to the VPE Gryo Basic Tuning register.
	///
	/// \param[in] fields The register's fields.
	/// \param[in] waitForReply Indicates if the method should wait for a response from the sensor.
	void writeVpeGryoBasicTuning(VpeGryoBasicTuningRegister &fields, bool waitForReply = true);
	...
	/// \param[in] adaptiveTuning Value for the AdaptiveTuning field.
	/// \param[in] waitForReply Indicates if the method should wait for a response from the sensor.
	void writeVpeGryoBasicTuning(
		const math::vec3f &angularWalkVariance,
		const math::vec3f &baseTuning,
		const math::vec3f &adaptiveTuning,
		bool waitForReply = true);
	...
	///
	/// \return The register's values.
	math::vec3f readFilterStartupGyroBias();

	/// \brief Writes to the Filter Startup Gyro Bias register.
	///
	/// \param[in] bias The register's Bias field.
	/// \param[in] waitForReply Indicates if the method should wait for a response from the sensor.
	void writeFilterStartupGyroBias(const math::vec3f &bias, bool waitForReply = true);
