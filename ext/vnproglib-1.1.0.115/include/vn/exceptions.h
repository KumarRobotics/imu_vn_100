...
public:
	#if VN_SUPPORTS_CSTR_STRING_CONCATENATE
	explicit not_found(std::string itemName) : runtime_error(std::string("Item '" + itemName + "' not found.").c_str()) { }
	#else
	explicit not_found(std::string itemName) : runtime_error("Item not found.") { }
	#endif
};
