...
/*void VnSensor::changeBaudrate(uint32_t br)
{
	if (_pi->pSerialPort == NULL)
		// We are not connected to a known serial port.
		throw invalid_operation();

	_pi->pSerialPort->changeBaudrate(br);
}*/
...
VpeGryoBasicTuningRegister VnSensor::readVpeGryoBasicTuning()
{
	char toSend[17];

	size_t length = Packet::genReadVpeGryoBasicTuning(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend));

	Packet response;
	_pi->transactionNoFinalize(toSend, length, true, &response);

	VpeGryoBasicTuningRegister reg;
	response.parseVpeGryoBasicTuning(
		&reg.angularWalkVariance,
		&reg.baseTuning,
		&reg.adaptiveTuning);
...
void VnSensor::writeVpeGryoBasicTuning(VpeGryoBasicTuningRegister &fields, bool waitForReply)
{
	char toSend[256];

	size_t length = Packet::genWriteVpeGryoBasicTuning(_pi->_sendErrorDetectionMode, toSend, sizeof(toSend), fields.angularWalkVariance, fields.baseTuning, fields.adaptiveTuning);

	Packet response;
	_pi->transactionNoFinalize(toSend, length, waitForReply, &response);
}

void VnSensor::writeVpeGryoBasicTuning(
	...
	bool waitForReply)
{
	...
	size_t length = Packet::genWriteVpeGryoBasicTuning(
		_pi->_sendErrorDetectionMode,
		toSend,
		sizeof(toSend),
		angularWalkVariance,
