...
struct PacketFinder::Impl
{
	...
	struct BinaryTracker
	{
		...
		explicit BinaryTracker(size_t possibleStartIndex, size_t runningDataIndex, TimeStamp foundTime) :
		...
		bool operator==(const BinaryTracker &rhs)
		{
			return
				...
				timeFound._sec == rhs.timeFound._sec &&
				timeFound._usec == rhs.timeFound._usec;
		}
	};
			...
			for (list<BinaryTracker>::iterator it = _binaryOnDeck.begin(); it != _binaryOnDeck.end(); ++it)
			{
				...
				if (ez.numOfBytesRemainingForCompletePacket == 0)
				{
					...
					else
					{
						...
						dispatchPacket(p, packetStartRunningIndex, ez.timeFound);
	...
	void dispatchPacket(Packet &packet, size_t runningDataIndexAtPacketStart, TimeStamp timestamp)
	{
		...
		//using namespace std;
		//cout << "Made Here" << endl;
		if (_pythonPacketFoundHandler != NULL)
		{
		//#vector<char> pRawData(readBuffer, readBuffer + numOfBytesRead);

			//python::AcquireGIL scopedLock;

			//_pythonPacketFoundHandler.
			//_pythonPacketFoundHandler();
			boost::python::call<void>(_pythonPacketFoundHandler, packet);
