...
struct SerialPort::Impl
{
	...
	void externalStop()
	{
		ExternalStopRequest = true;
	}

	void externalStopFinished()
	{
		ExternalStopRequest = false;
	}
	...
	/*void AlertSerialPortNotificationsThreadOfBaudrateChange()
	{
		ChangingBaudrate = true;

		ContinueHandlingSerialPortEvents = false;

		NotificationsThreadStopped.wait();
...
{
	#if PYTHON && PL156_FIX_ATTEMPT_1
	// Should this be in VnSensor disconnect or here? -Paul
	python::ReleaseGIL t;
	#endif
